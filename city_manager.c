#include <stdio.h>
#include <string.h>
#include <sys/stat.h>
#include <sys/types.h>
#include <errno.h>
#include <fcntl.h>
#include <unistd.h>
#include <time.h>
#include <stdlib.h>
#include <sys/wait.h>   
#include <signal.h>  
#define PID_FILE ".monitor_pid"

// Structura principala pentru rapoarte
typedef struct {
    int id;
    char inspector[50];
    float latitude;
    float longitude;
    char category[30];
    int severity;
    time_t timestamp;
    char description[256];
} Report;

// Structura pentru filtre
typedef struct {
    char field[30];
    char op[5];
    char value[50];
} Condition;



//////////////////////////////////////////////////////////// verificare permisiuni ////////////////////////////////////////////////////////////////////////
int verifica_permisiuni(const char *cale, const char *rol, int scriere) {
    struct stat st;
    if (stat(cale, &st) == -1) return 1; 

   if (strcmp(rol, "manager") == 0) {
    /* Cazul pentru OWNER (User) */
        if (scriere) {
            // Verifică dacă bitul pentru scriere user este activ
            return (st.st_mode & S_IWUSR);
        } else {
            // Verifică dacă bitul pentru citire user este activ
            return (st.st_mode & S_IRUSR);
        }
    } 
    else {
        /* Cazul pentru GRUP (Group) */
        if (scriere) {
            // Verifică dacă bitul pentru scriere grup este activ
            return (st.st_mode & S_IWGRP);
        } else {
            // Verifică dacă bitul pentru citire grup este activ
            return (st.st_mode & S_IRGRP);
        }
    }
}

///////////////////////////////////////////// Transformam bitii de mod in string-ul clasic rw-rw-r-- /////////////////////////////////////////////////
void mode_to_string(mode_t mode, char *str) {
    strcpy(str, "---------");
    if (mode & S_IRUSR) str[0] = 'r';
    if (mode & S_IWUSR) str[1] = 'w';
    if (mode & S_IXUSR) str[2] = 'x';
    if (mode & S_IRGRP) str[3] = 'r';
    if (mode & S_IWGRP) str[4] = 'w';
    if (mode & S_IXGRP) str[5] = 'x';
    if (mode & S_IROTH) str[6] = 'r';
    if (mode & S_IWOTH) str[7] = 'w';
    if (mode & S_IXOTH) str[8] = 'x';
}

////////////////////////////////////////// Inregistram actiunea in log-ul districtului ////////////////////////////////////////////////////////////////////
void scrie_in_log(const char *cale_log, const char *rol, const char *user, const char *actiune) {

    // Verificăm dacă avem voie să scriem (inspectorii nu pot)
    if (!verifica_permisiuni(cale_log, rol, 1)) {
        printf("Restrictie: Rolul '%s' nu are dreptul de a scrie in %s.\n", rol, cale_log);
        return;
    }

    int fd = open(cale_log, O_CREAT | O_WRONLY | O_APPEND, 0644);
    chmod(cale_log, 0644); // Ne asigurăm că are exact rw-r--r-- conform
    
    if (fd != -1) {
        time_t acum = time(NULL);
        char *timp = ctime(&acum);
        timp[strlen(timp) - 1] = '\0'; 

        char buffer[512];
        int n = sprintf(buffer, "[%s] %s (%s) -> %s\n", timp, user, rol, actiune);
        write(fd, buffer, n);
        close(fd);
    }
}


////////////////////////////////////////////////////// Functii pentru filtrare  ////////////////////////////////////////////////////////////////////////////
int parseaza_conditie(const char *input, Condition *c) {
    return (sscanf(input, "%29[^:]:%4[^:]:%49s", c->field, c->op, c->value) == 3);
}

int verifica_match(Report r, Condition c) {
    if (strcmp(c.field, "severity") == 0) {
        int v = atoi(c.value);
        if (strcmp(c.op, "=") == 0) return r.severity == v;
        if (strcmp(c.op, ">") == 0) return r.severity > v;
        if (strcmp(c.op, "<") == 0) return r.severity < v;
        if (strcmp(c.op, ">=") == 0) return r.severity >= v;
        if (strcmp(c.op, "<=") == 0) return r.severity <= v;
    } else if (strcmp(c.field, "category") == 0) {
        if (strcmp(c.op, "=") == 0) return strcmp(r.category, c.value) == 0;
    }
    return 0;
}

////////////////////////////////////////////////////// --- SUBPROGRAME PENTRU COMENZI ---/////////////////////////////////////////////////////////////////////////

void comanda_add(const char *dist, const char *rol, const char *user) {
    

    // Citim datele de la tastatura 
    Report r;
    printf("X(latitudine): "); scanf("%f", &r.latitude);
    printf("Y(logitudine): "); scanf("%f", &r.longitude);

    // Validare Categorie
    int categorie_valida = 0;
    while (!categorie_valida) {
        printf("Categorie (road/lighting/flooding/other): "); 
        scanf("%29s", r.category); // Limitam la 29 caractere pentru a nu depasi buffer-ul de 30
        
        if (strcmp(r.category, "road") == 0 || 
            strcmp(r.category, "lighting") == 0 || 
            strcmp(r.category, "flooding") == 0 || 
            strcmp(r.category, "other") == 0) {
            categorie_valida = 1;
        } else {
            printf("Eroare: Categorie invalida! Te rog scrie exact unul dintre cuvintele din paranteza.\n");
        }
    }

    // Validare Severity Level
    int severitate_valida = 0;
    while (!severitate_valida) {
        printf("Severity level (1/2/3): ");
       
        if (scanf("%d", &r.severity) == 1) {  // scanf returneaza 1 daca a citit cu succes un integer
            if (r.severity >= 1 && r.severity <= 3) {
                severitate_valida = 1;
            } else {
                printf("Eroare: Severitatea trebuie sa fie strict 1, 2 sau 3.\n");
            }
        } else {
            printf("Eroare: Te rog introdu un numar valid!\n");
            int c;
            while ((c = getchar()) != '\n' && c != EOF); //curatam linia
        }
    }
    
    getchar(); // curatam buffer-ul de '\n' ramas de la ultimul scanf inainte de fgets
    printf("Descriere: "); fgets(r.description, 256, stdin);
    r.description[strlen(r.description)-1] = '\0';


    ///////////////////////////creearea p-zisa
    //cai ctre fisiere
    char path[256], link[256], log[256];
    sprintf(path, "%s/reports.dat", dist);
    sprintf(link, "active_reports-%s", dist);
    sprintf(log, "%s/logged_district", dist);

    //director cu permisiuni
    mkdir(dist, 0750);//rwxr-x---
    chmod(dist, 0750);

    //verificare 
    if (!verifica_permisiuni(path, rol, 1)) {
        printf("Acces refuzat la scriere!\n");
        return;
    }

    //deschidere fisier binar
    int fd = open(path, O_CREAT | O_RDWR, 0664);//rw-rw-r--
    chmod(path, 0664);

    // Calcul id 
    off_t size = lseek(fd, 0, SEEK_END);
    r.id = (size / sizeof(Report)) + 1;
    r.timestamp = time(NULL);
    strncpy(r.inspector, user, 50);

    write(fd, &r, sizeof(Report));
    close(fd);

    // Crearea fisierului de configurare 
    char config_path[256];
    sprintf(config_path, "%s/district.cfg", dist);
    
    // Folosim O_EXCL pentru a-l crea doar daca nu exista deja
    int cfg_fd = open(config_path, O_CREAT | O_WRONLY | O_EXCL, 0640);
    if (cfg_fd != -1) {
        write(cfg_fd, "2", 1); // Setam un prag de severitate default de 2
        close(cfg_fd);
    }
    chmod(config_path, 0640); // Ne asiguram ca permisiunile sunt 640

    // Actualizam link-ul simbolic 
    unlink(link); 
    symlink(path, link);

    scrie_in_log(log, rol, user, "Adaugare raport");
    printf("Raport %d salvat cu succes.\n", r.id);





    /////// faza 2
    int monitor_informat = 0;
    FILE *f_pid = fopen(PID_FILE, "r");
    
    if (f_pid != NULL) {
        pid_t monitor_pid;
        if (fscanf(f_pid, "%d", &monitor_pid) == 1) {
            // Trimitem semnalul SIGUSR1 procesului citit
            if (kill(monitor_pid, SIGUSR1) == 0) {
                monitor_informat = 1;
            }
        }
        fclose(f_pid);
    }

    // Pregătim mesajul de log în funcție de succesul notificării
    char log_path[256];
    sprintf(log_path, "%s/logged_district", dist);

    if (monitor_informat) {
        scrie_in_log(log_path, rol, user, "Raport adaugat - monitor informat prin SIGUSR1");
    } else {
        // Dacă fișierul .monitor_pid nu există sau kill() a eșuat
        scrie_in_log(log_path, rol, user, "Raport adaugat - monitorul NU a putut fi informat");
    }
}

void comanda_list(const char *dist, const char *rol, const char *user) {
    char path[256], log[256];
    sprintf(path, "%s/reports.dat", dist);
    sprintf(log, "%s/logged_district", dist);

    struct stat st;
    if (stat(path, &st) == -1) {
        printf("District inexistent.\n");
        return;
    }

    char p_str[11];
    mode_to_string(st.st_mode, p_str);
    printf("Fisier: %s | Permisiuni: %s | Marime: %ld bytes | Last Mod: %s", 
           path, p_str, st.st_size, ctime(&st.st_mtime));

    int fd = open(path, O_RDONLY);
    Report r;
    
   
    printf("\nID  | Inspector  | Categorie  | Sev | Descriere\n");
    printf("--------------------------------------------------\n");
    while (read(fd, &r, sizeof(Report)) > 0) {
        printf("%-3d | %-10s | %-10s | %d   | %s\n", 
               r.id, r.inspector, r.category, r.severity, r.description);
    }
    close(fd);
    scrie_in_log(log, rol, user, "Listare rapoarte");
}

void comanda_remove_report(const char *dist, const char *rol, const char *user, int target_id) {

    if (strcmp(rol, "manager") != 0) {
        printf("Doar managerul poate sterge!\n");
        return;
    }
   
    char path[256], log[256]; 
    
    sprintf(path, "%s/reports.dat", dist);// calea către fișierul de reports
    sprintf(log, "%s/logged_district", dist); // calea către fișierul de log

    int fd = open(path, O_RDWR);
    if (fd == -1) return;


    Report r;
    off_t pos = -1;
    while (read(fd, &r, sizeof(Report)) > 0) {
        if (r.id == target_id) {
            pos = lseek(fd, 0, SEEK_CUR) - sizeof(Report);
            break;
        }
    }

    if (pos != -1) {
        Report urm;
        while (read(fd, &urm, sizeof(Report)) > 0) {
            off_t current = lseek(fd, 0, SEEK_CUR);
            lseek(fd, pos, SEEK_SET);
            write(fd, &urm, sizeof(Report));
            pos = lseek(fd, 0, SEEK_CUR);
            lseek(fd, current, SEEK_SET);
        }
        struct stat st;
        fstat(fd, &st);
        ftruncate(fd, st.st_size - sizeof(Report));
        printf("Raport sters cu succes.\n");
        close(fd);
    }
    char mesaj_log[100];
    sprintf(mesaj_log, "Stergere raport cu ID-ul %d", target_id);
    scrie_in_log(log, rol, user, mesaj_log);
}
void comanda_view(const char *dist, const char *rol, const char *user, int target_id) {
    char path[256], log[256];
    sprintf(path, "%s/reports.dat", dist);
    sprintf(log, "%s/logged_district", dist);

    // 1. Verificăm dacă rolul curent are permisiunea de CITIRE (parametrul de scriere = 0)
    if (!verifica_permisiuni(path, rol, 0)) {
        printf("Acces refuzat: Rolul '%s' nu are permisiunea de a vizualiza rapoartele.\n", rol);
        return;
    }

    // 2. Deschidem fișierul doar pentru citire
    int fd = open(path, O_RDONLY);
    if (fd == -1) {
        perror("Eroare la deschiderea fisierului cu rapoarte");
        return;
    }

    Report r;
    int gasit = 0;

    // 3. Citim înregistrările una câte una până găsim ID-ul căutat
    while (read(fd, &r, sizeof(Report)) > 0) {
        if (r.id == target_id) {
            printf("\n--- Detalii Raport %d ---\n", r.id);
            printf("Inspector:   %s\n", r.inspector);
            printf("Coordonate:  %f, %f\n", r.latitude, r.longitude);
            printf("Categorie:   %s\n", r.category);
            printf("Severitate:  %d\n", r.severity);
            printf("Timestamp:   %s", ctime(&r.timestamp));
            printf("Descriere:   %s\n", r.description);
            printf("---------------------------\n");
            
            gasit = 1;
            break; // Oprim căutarea după ce l-am găsit
        }
    }

    if (!gasit) {
        printf("Raportul cu ID-ul %d nu a fost gasit in districtul %s.\n", target_id, dist);
    }

    close(fd);

    // 4. Înregistrăm acțiunea în log
    char mesaj_log[100];
    sprintf(mesaj_log, "Vizualizare raport %d", target_id);
    scrie_in_log(log, rol, user, mesaj_log);
}
void comanda_update_threshold(const char *dist, const char *rol, const char *user, const char *valoare) {
    // 1. Verificăm rolul: Doar managerii au voie să modifice threshold-ul
    if (strcmp(rol, "manager") != 0) {
        printf("Acces refuzat: Doar rolul 'manager' poate actualiza pragul de severitate.\n");
        return;
    }

    char path[256], log[256];
    sprintf(path, "%s/district.cfg", dist);
    sprintf(log, "%s/logged_district", dist);

    // 2. Extra Cerință: Apelăm stat() și verificăm dacă permisiunile sunt exact 640
    struct stat st;
    if (stat(path, &st) == 0) {
        // st.st_mode conține și tipul fișierului, așa că folosim & 0777 pentru a izola doar biții de permisiuni
        if ((st.st_mode & 0777) != 0640) {
            printf("Diagnostic: Permisiunile fisierului %s nu sunt 640! Refuzam suprascrierea.\n", path);
            return;
        }
    } else {
        perror("Eroare la citirea metadatelor pentru district.cfg (Poate districtul nu exista?)");
        return;
    }

    // 3. Suprascriem fișierul cu noua valoare
    int fd = open(path, O_WRONLY | O_CREAT | O_TRUNC, 0640);
    if (fd != -1) {
        write(fd, valoare, strlen(valoare));
        close(fd);
        printf("Pragul de severitate a fost actualizat la %s pentru districtul %s.\n", valoare, dist);
        
        // 4. Jurnalizăm operațiunea
        scrie_in_log(log, rol, user, "Actualizare threshold district.cfg");
    } else {
        perror("Eroare la deschiderea fisierului de configurare");
    }
}
void comanda_filter(const char *dist, const char *rol, const char *user, const char *conditie_str) {
    char path[256], log[256];
    sprintf(path, "%s/reports.dat", dist);
    sprintf(log, "%s/logged_district", dist);

    // 1. Verificăm dacă utilizatorul are drept de citire
    if (!verifica_permisiuni(path, rol, 0)) {
        printf("Acces refuzat: Rolul '%s' nu are permisiunea de a citi rapoartele.\n", rol);
        return;
    }

    // 2. Parsăm condiția primită ca argument folosind funcția ta
    Condition cond;
    if (!parseaza_conditie(conditie_str, &cond)) {
        printf("Eroare: Formatul conditiei este invalid. Foloseste formatul field:operator:value\n");
        return;
    }

    // 3. Deschidem fișierul binar
    int fd = open(path, O_RDONLY);
    if (fd == -1) {
        perror("Eroare la deschiderea fisierului de rapoarte");
        return;
    }

    Report r;
    int gasit = 0;
    
    printf("\n--- Rezultate Filtrare (%s) ---\n", conditie_str);
    printf("ID  | Categorie  | Sev | Descriere\n");
    printf("--------------------------------------------------\n");

    // 4. Citim și testăm fiecare înregistrare
    while (read(fd, &r, sizeof(Report)) > 0) {
        if (verifica_match(r, cond)) { // Folosim a doua funcție a ta
            printf("%-3d | %-10s | %-3d | %s\n", 
                   r.id, r.category, r.severity, r.description);
            gasit = 1;
        }
    }

    if (!gasit) {
        printf("Niciun raport nu se potriveste cu criteriul specificat.\n");
    }

    close(fd);

    // 5. Jurnalizăm operațiunea[cite: 1]
    char mesaj_log[200];
    sprintf(mesaj_log, "Filtrare rapoarte dupa conditia: %s", conditie_str);
    scrie_in_log(log, rol, user, mesaj_log);
}

void comanda_remove_district(const char *dist, const char *rol, const char *user) {


    if (strcmp(rol, "manager") != 0) {
        printf("Doar managerul poate sterge un district!\n");
        return;
    }

     struct stat st;
    //verificare st;
    if (stat(dist, &st) == -1) {
        fprintf(stderr, "Eroare: Districtul '%s' nu exista.\n", dist);
        return;
    }
    //verificare ca e director
    if (!S_ISDIR(st.st_mode)) {
        fprintf(stderr, "Eroare: '%s' nu este un director.\n", dist);
        return;
    }

    pid_t pid = fork();
    if (pid == 0) {
        // Suntem în procesul fiu
        execlp("rm", "rm", "-rf", dist, NULL);
        perror("Eroare la exec"); // Se execută doar dacă execlp eșuează
        exit(1);
    } else {
        // Suntem în părinte
        wait(NULL); // Așteptăm să termine rm -rf
        char link_name[256];
        sprintf(link_name, "active_reports-%s", dist);
        unlink(link_name);
        printf("Districtul %s si link-ul sau au fost sterse.\n", dist);
    }
}


// // Handler pentru SIGUSR1 - Notificare raport nou
// void handle_sigusr1(int sig) {
//     const char *msg = "\n[Monitor] Alert: A new report has been added to the system!\n";
//     write(STDOUT_FILENO, msg, strlen(msg));
// }

// // Handler pentru SIGINT - Închidere program 
// void handle_sigint(int sig) {
//     const char *msg = "\n[Monitor] Shutting down. Cleaning up PID file...\n";
//     write(STDOUT_FILENO, msg, strlen(msg));
//     unlink(PID_FILE); // Șterge fișierul la ieșire 
//     exit(0);
// }
//////////////////////////////////////////////////////////////// MAIN //////////////////////////////////////////////////////////////////////////////

int main(int argc, char *argv[]) {
    char *role = "inspector"; // Rolul  
    char *user = "anonim";    // Numele utilizatorului 
    char *dist = NULL;        // Districtul pe care lucram 
    char *cmd = NULL;         // Comanda pe care o vom executa 
    char *extra = NULL;       // Pentru argumente extra, cum e ID-ul raportului 


    for (int i = 1; i < argc; i++) {
        
        // 1. setarile globale (role si user)
        if (strcmp(argv[i], "--role") == 0) {
            if (i + 1 < argc) {
                role = argv[i + 1];
                i++; // Am consumat si urmatorul argument, deci trecem peste el
            }
        } 
        else if (strcmp(argv[i], "--user") == 0) {
            if (i + 1 < argc) {
                user = argv[i + 1];
                i++;
            }
        } 
        
        // 2. ce comanda vrea sa execute utilizatorul
        else if (strcmp(argv[i], "--add") == 0) {
            if (i + 1 < argc) {
                cmd = "add";
                dist = argv[i + 1];
                i++;
            }
        } 
        else if (strcmp(argv[i], "--list") == 0) {
            if (i + 1 < argc) {
                cmd = "list";
                dist = argv[i + 1];
                i++;
            }
        } 
        else if (strcmp(argv[i], "--remove_report") == 0) {
            //  district SI de ID-ul raportului 
            if (i + 2 < argc) {
                cmd = "remove_report";
                dist = argv[i + 1];
                extra = argv[i + 2];
                i += 2; // Am consumat districtul si ID-ul, deci sarim 2 pozitii
            }
        } 
        else if (strcmp(argv[i], "--remove_district") == 0) {
            if (i + 1 < argc) {
                cmd = "remove_district"; 
                dist = argv[i + 1];
                i++;
            }
        }
        else if (strcmp(argv[i], "--view") == 0) {
            // --view are nevoie de district SI de ID-ul raportului
            if (i + 2 < argc) {
                cmd = "view";
                dist = argv[i + 1];
                extra = argv[i + 2];
                i += 2; // Sărim peste district și id
            }
        }
        else if (strcmp(argv[i], "--update_threshold") == 0) {
            if (i + 2 < argc) {
                cmd = "update_threshold";
                dist = argv[i + 1];
                extra = argv[i + 2]; // Aici 'extra' va fi valoarea threshold-ului
                i += 2; 
            }
        }
        else if (strcmp(argv[i], "--filter") == 0) {
            if (i + 2 < argc) {
                cmd = "filter";
                dist = argv[i + 1];
                extra = argv[i + 2]; // Aici 'extra' va fi string-ul conditiei
                i += 2;
            }
        }
    }

    // Daca nu avem comanda sau district, nu avem ce face, deci oprim programul
    if (cmd == NULL || dist == NULL) {
        printf("Utilizare: ./city_manager --role [rol] --user [nume] --[comanda] [district] [optional: id]\n");
        return 0;
    }

    char link_name[256];
    sprintf(link_name, "active_reports-%s", dist);
    
    struct stat link_stat;
    // Verificam link-ul simbolic
    if (lstat(link_name, &link_stat) == 0) {
        if (S_ISLNK(link_stat.st_mode)) {
            struct stat target_stat;
            // stat() urmeaza link-ul; daca pica, link-ul e rupt (dangling)
            if (stat(link_name, &target_stat) == -1) {
                printf("Warning: Link-ul simbolic %s este rupt (indica spre ceva ce nu exista)!\n", link_name);
            }
        }
    }

    // Executăm subprogramul corespunzător comenzii alese
    if (strcmp(cmd, "add") == 0) {
        comanda_add(dist, role, user);
    } 
    else if (strcmp(cmd, "list") == 0) {
        comanda_list(dist, role, user);
    } 
    else if (strcmp(cmd, "remove_report") == 0) {
        comanda_remove_report(dist, role, user, atoi(extra));
    } 
    else if (strcmp(cmd, "remove_district") == 0) {
        comanda_remove_district(dist, role, user); 
    }
    else if (strcmp(cmd, "view") == 0) {
        if (extra == NULL) {
            printf("Eroare: Trebuie sa specifici un ID de raport pentru comanda --view.\n");
        } else {
            comanda_view(dist, role, user, atoi(extra));
        }
    }
    else if (strcmp(cmd, "update_threshold") == 0) {
        if (extra == NULL) {
            printf("Eroare: Trebuie sa specifici o valoare pentru threshold (--update_threshold <district> <valoare>).\n");
        } else {
            comanda_update_threshold(dist, role, user, extra);
        }
    }
    else if (strcmp(cmd, "filter") == 0) {
        if (extra == NULL) {
            printf("Eroare: Trebuie sa specifici o conditie (--filter <district> <field:op:value>).\n");
        } else {
            comanda_filter(dist, role, user, extra);
        }
    }

    return 0;
}